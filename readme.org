...
- Gestures: includes MotionEvent for single finger, double or three fingers, use pointers, pack different gestures tasks into different runnables. gestures should be easy to use and apply. Look back on DrawingFun fun app developped during Fall 2014, fragments, threads, view/invalidates were all so easy, don't Understand why during those days I could only make them work without really/deeply understood how they works that way. 
  - In total about 14 gestions that I could think so far
...
*** Status Update
- My app is not stable yet ever since Wednesday evening rotating around block center, tried to debug, but it came back alive so I just continued with simple gesture tests. But will work on debugging next time when it happens. 
- Will continue with some search online for confirm if ray picking or not, and will at least try ray-pickign on the app in the evening. 
- I guess I will have to read some design patterns books. Feel I am slightly week on this side. 
- The app crashed last night though first appeared on Wednesday evening before block rotated around its center finished. It seems that my Studio debug has not been appropriately set yet, will work on it in the afternoon. Work on simple gestures to rotate frmames and current active block in the afternoon if app crashes got fixed. 
- Will update at 11:17pm or 12:17pm, or slightly earlier or later depends on progress made. 
- 
