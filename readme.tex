...
\label{sec-1-1}
\begin{itemize}
\item Slightly cleaned version of code.
\item Due to sleep/rest problems, I am NOT in a good clear mind today, would try ray picking on gestures matrix side, but if I feel my mind can not keep up today, will try to work on game flow labor work then.
\item Would spend time go to campus checking about potential possibility for my scholarship/tuition solutions, do lundry, cook some good food for dinner etc, adjust myself so that tomorrow when my mind could be better I could finish all most if not all the technical challenging part.
\item May not necessarily update two more times today depends on progress.
