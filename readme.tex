...
\label{sec-1}
\subsection{OpenGL 3D tetris status}
\label{sec-1-1}
\begin{itemize}
\item I didn't do anything, but Emacs + Auctex + org-mode export pdf Chinese just displayed as I wished\textasciitilde{}~!!!!
\item Will work on Cube project, and build a simple version of opengl tetris game from that app first, and come back to this one later.
\item This project will be ongoing secondary priority project fo recent days considering my first priority will be current course projects.
...
\item 3d: will work on a simple opengl 3d version first.
\item glar3d: opengl 3d adapt from tetrisglar app, will implement when simple 3d version is done. (After having understood texture and lights better, tried to debug this one for a while, but still complicated design and layout still make this one to some extend difficult for me for now.)
\end{itemize}
